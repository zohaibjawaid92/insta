...
slideOpts = {
  ...
  ngOnInit() {
    this.getFeeds();
    this.getFeedsPictures();
  }

  getFeeds(){
    this.feedService.getLatestFeeds().subscribe(x => {
      this.getFeedsInfo = x['results'];
      console.log(this.getFeedsInfo);
    })
  }

  getFeedsPictures(){
    this.feedService.getFeedsPictures().subscribe(data => {
      this.getPictures = data[0]['download_url'];
      console.log(this.getPictures);
    })
  }
